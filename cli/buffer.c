...
struct buffer_data *buffer_data_new (size_t size)
{
	...
	d = calloc(1,sizeof (struct buffer_data));
	if(!d)
		return NULL;
...
{
	if (b || !b->tail || b->tail->cp == b->tail->sp)
		return 1;
...
{
  buffer_write (b, (unsigned char *)&c, 2);
  return 1;
}
...
buffer_flush (struct buffer *b, int fd, size_t size)
{
  ...
  iovec = calloc(1,sizeof (struct iovec) * b->alloc);
  iov_index = 0;
		      ...
		      int no_more_flag)
{
  ...
  if (b->alloc == 1)
    ...
    iov = calloc (1,sizeof (struct iovec) * (b->alloc + 2));
...
/* Flush buffer to the file descriptor.  Mainly used from vty
   interface. */
int
buffer_flush_vty (struct buffer *b, int fd, unsigned long size, 
